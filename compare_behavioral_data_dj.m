...
mycolor = [27,158,119; 217,95,2; 117,112,179; 231,41,138;102,166,30;]./255; % û��, ��Ȳ, ����, ��ȫ, ����
...
for sub_i = 1:numel(subnames)
    ...
    dd = cell(5,1); % û��, ��Ȳ, ����, ��ȫ, ����val, self, time, vivid, safethreat
...
for sub_i = numel(subnames)%1:numel(subnames)
...
if numel(subnames) > 21
    for i = 1:numel(subnames)-21
    mycolor = [mycolor; mycolor(i, :)];
    end
end
...
%% mean and variance of correlation value btw type2&type3
  ...
  for sub_i = 1%:numel(subnames)
      ...
      for run_i = 1:numel(mr_run)
          ...
          for dd = 1:5 % valence, self, time, vivid, safe&threat
              ...
              % post-type2 (19 dim) "15 words" rating
              rating.post_type2_self(run_i,:) = survey.dat.response{1}(run_i,:); % self only
              rating.post_type2_impor(run_i,:) = survey.dat.response{2}(run_i,:); % importance only
              rating.post_type2_cent(run_i,:) = survey.dat.response{3}(run_i,:); % centrality only
              rating.post_type2_social(run_i,:) = survey.dat.response{4}(run_i,:); % social only
              rating.post_type2_past(run_i,:) = survey.dat.response{5}(run_i,:); % past only
              rating.post_type2_present(run_i,:) = survey.dat.response{6}(run_i,:); % present only
              rating.post_type2_future(run_i,:) = survey.dat.response{7}(run_i,:); % future only
              rating.post_type2_pos(run_i,:) = survey.dat.response{9}(run_i,:); % positive only
              rating.post_type2_neg(run_i,:) = survey.dat.response{10}(run_i,:); % negative only
              rating.post_type2_safe(run_i,:) = survey.dat.response{11}(run_i,:); % safe only
              rating.post_type2_threat(run_i,:) = survey.dat.response{12}(run_i,:); % threat only
              rating.post_type2_intensity(run_i,:) = survey.dat.response{15}(run_i,:); % intensity only
              rating.post_type2_vivid(run_i,:) = survey.dat.response{16}(run_i,:); % vivid only
              rating.post_type2_abstract(run_i,:) = survey.dat.response{17}(run_i,:); % abstract only
              % post-type3 (5 dim) rating
              for w = 1:15
                  rating.post_type3_all{dd, 1}(run_i,w) = survey2.survey.dat{run_i,w}{dd}.rating;
              end
          end
      end
      
          C(sub_i, 1) = corr(reshape(rating.post_type2_self, 60, 1), reshape(rating.post_type3_all{2}, 60, 1));
          C(sub_i, 2) = corr(reshape(rating.post_type2_impor, 60, 1), reshape(rating.post_type3_all{2}, 60, 1));
          C(sub_i, 3) = corr(reshape(rating.post_type2_cent, 60, 1), reshape(rating.post_type3_all{2}, 60, 1));
          C(sub_i, 4) = corr(reshape(rating.post_type2_social, 60, 1), reshape(rating.post_type3_all{2}, 60, 1));
          
          C(sub_i, 5) = corr(reshape(rating.post_type2_past, 60, 1), reshape(rating.post_type3_all{3}, 60, 1));
          C(sub_i, 6) = corr(reshape(rating.post_type2_present, 60, 1), reshape(rating.post_type3_all{3}, 60, 1));
          C(sub_i, 7) = corr(reshape(rating.post_type2_future, 60, 1), reshape(rating.post_type3_all{3}, 60, 1));
          
          C(sub_i, 8) = corr(reshape(rating.post_type2_pos, 60, 1), reshape(rating.post_type3_all{1}, 60, 1));
          C(sub_i, 9) = corr(reshape(rating.post_type2_neg, 60, 1), reshape(rating.post_type3_all{1}, 60, 1));
          C(sub_i, 10) = corr(reshape(rating.post_type2_intensity, 60, 1), reshape(rating.post_type3_all{1}, 60, 1));
          
          C(sub_i, 11) = corr(reshape(rating.post_type2_safe, 60, 1), reshape(rating.post_type3_all{5}, 60, 1));
          C(sub_i, 12) = corr(reshape(rating.post_type2_threat, 60, 1), reshape(rating.post_type3_all{5}, 60, 1));
          
          C(sub_i, 13) = corr(reshape(rating.post_type2_vivid, 60, 1), reshape(rating.post_type3_all{4}, 60, 1));
          C(sub_i, 14) = corr(reshape(rating.post_type2_abstract, 60, 1), reshape(rating.post_type3_all{4}, 60, 1));
          
  end   
  
  
  for i = 1:14
      C(sub_i + 1, i) = mean(C(:, i));
      C(sub_i + 2, i) = var(C(:, i));
  end
