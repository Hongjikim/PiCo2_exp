...
%% KOREAN INSTRUCTIONS
msg.hs_dc = double('?¤ì??? ì¡°ì?? ????ì¤???????.\n ?????? ë°????? ?? ???µë????. ??ë©? ì¤????? ????????ë¥?\n ?¸ì???? ë§????¼ë? ë°??¼ë?ì£¼ì?¸ì??.'); % head scout and distortion correction
msg.inst1 = double('?´ë? ?¸ì???? ???? ??ê°? ?¸ì????????. \n ??ë©´ì?? + ????ê°? ??????ë©?, ???? ??ê°??? ???????¸ì??. \n + ????ê°? ?¬ë?¼ì? ??ë§??? ì§???ë¬¸ì?? ?µë??? ?´ì£¼?¸ì??.\n\n ?°ì?µì?? ??ë²? ?´ë³´ê²??µë????.') ;
msg.inst2 = double('?????¨ì?µë????. ?¸ì???? ??????ê²??µë????.');


msg.s_key = double('ì°¸ê???ê°? ì¤?ë¹??????¼ë©´, \n ?´ë?¸ì??? ?????©ë???? (s).');
msg.s_key2 = double('?´ë? ?¸ì???? ???? ??ê°? ?¸ì????????. \n ??ë©´ì?? + ????ê°? ??????ë©?, ???? ??ê°??? ???????¸ì??. ...\n + ????ê°? ?¬ë?¼ì? ??ë§??? ì§???ë¬¸ì?? ?µë??? ?´ì£¼?¸ì??.\n\n ì°¸ê???ê°? ì¤?ë¹??????¼ë©´ ?´ë?¸ì??? ?????©ë????. (s)') ;

msg.start_buffer = double('?????©ë????...');

msg.ThoughtSampling = double('ì§?ê¸? ë¬´ì?? ??ê°??? ??ê³? ????ì§? \n ?¨ì?´ë?? êµ¬ë? ë§??´ì£¼?¸ì??.');
msg.fixation = double('+');

msg.postQ_inst = double('?´ë? ?¸ì???? ???¬ì?µë????. \n ???????? ì§?ë¬¸ë?¤ì?? ?µë??´ì£¼?¸ì??.');
msg.run_end = double('?????¨ì?µë????. ???? ??ê¸°í?? ì£¼ì?¸ì??.');
%% FULL SCREEN
...
rec=[lb,tb; lb+recsize(1),tb; lb,tb+recsize(2); lb+recsize(1),tb+recsize(2);
    lb,tb+2*recsize(2); lb+recsize(1),tb+2*recsize(2)]; % 6ê°? ?¬ê????? ?¼ìª½ ?? ê¼?ì§????? ì¢???
...
Screen(theWindow, 'FillRect', bgcolor, window_rect);
intro_prompt1 = double('ë°©ê? ???? ??ê°? ê³¼ì??ë¥? ???????? ???°ì?¤ë?½ê? ???¬ë¦° ??ê°??? ???? ì§?ë¬¸ì??????.') ;
